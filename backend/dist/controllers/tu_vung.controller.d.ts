import { Request, Response } from "express";
export declare const xuLyLayDanhSachTu: (yeuCau: Request, phanHoi: Response) => Promise<Response<any, Record<string, any>>>;
export declare const xuLyThemTu: (yeuCau: Request, phanHoi: Response) => Promise<Response<any, Record<string, any>>>;
export declare const xuLyThemNhieuTu: (yeuCau: Request, phanHoi: Response) => Promise<Response<any, Record<string, any>>>;
export declare const xuLyCapNhatTu: (yeuCau: Request, phanHoi: Response) => Promise<Response<any, Record<string, any>>>;
export declare const xuLyXoaTu: (yeuCau: Request, phanHoi: Response) => Promise<Response<any, Record<string, any>>>;
export declare const xuLyLuuPhienHoc: (yeuCau: Request, phanHoi: Response) => Promise<Response<any, Record<string, any>>>;
export declare const xuLyLayTuCuonChieu: (yeuCau: Request, phanHoi: Response) => Promise<Response<any, Record<string, any>>>;
export declare const xuLyDiChuyenTu: (yeuCau: Request, phanHoi: Response) => Promise<Response<any, Record<string, any>>>;
export declare const xuLyLayTuThongMinh: (yeuCau: Request, phanHoi: Response) => Promise<Response<any, Record<string, any>>>;
export declare const xuLyLayTuDaThuoc: (yeuCau: Request, phanHoi: Response) => Promise<Response<any, Record<string, any>>>;
export declare const xuLyLayTienDoThuMuc: (yeuCau: Request, phanHoi: Response) => Promise<Response<any, Record<string, any>>>;
export declare const xuLyChamDiemCau: (yeuCau: Request, phanHoi: Response) => Promise<Response<any, Record<string, any>>>;
//# sourceMappingURL=tu_vung.controller.d.ts.map